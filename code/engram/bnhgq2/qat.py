"""Native HGQ2 QAT — the Quantization-aware training model .
...
Why a CUSTOM weight quantizer (verified 2026-07-07, LEDGER):c
  ...
  (probe T1: 4096/4096 latents -> 0). That is TERNARY and a STOP condition (the binary-weight model
...
class PairFeatures(keras.layers.Layer):
    ...
    learns it). When input_mu/input_sigma are given (the reference experiment input_std contract, where
...
@keras.saving.register_keras_serializable(package="bnhgq2")
class MeanMonoL1(keras.regularizers.Regularizer):
    """Keep a site's width penalty invariant when scalar grids become channel grids."""
    def __init__(self, l1=1e-8):
        self.l1 = float(l1)

    def __call__(self, x):
        return self.l1 * ops.mean(x)

    def get_config(self):
        return {"l1": self.l1}


def _free_act(act_bits: int, i0: int, bw_l1: float = 1e-8, heterogeneous_axis=()):
    """LEARNABLE-BITWIDTH activation quantizer (reference experiment / jsc150 style).

    Stock HGQ2 KIF datalane quantizer with `trainable=True` and a MonoL1 regulariser on
    BOTH bitwidth parameters (i = integer bits, f = fractional bits), initialised on the
    `act_bits` grid that `calibrate_activations` picks (i from MSE, f = bits-1-i). Under
    `BetaScheduler`+`beta > 0` the EBOPs term then pays for every bit the network keeps,
    so the widths anneal DOWN during training.

    This is NOT `_trainable_act`: that one pins the total width via a Constant constraint
    and trains only the scale (integer/fractional split). Here the width itself is the
    free variable, which is the whole point of the EBOPs-pressure experiment.

    SAT/RND_CONV (not the HGQ2 WRAP default) so the trained grid is the deployable grid,
    exactly as in `_static_act`/`_trainable_act` (train == deploy)."""
    from hgq.constraints import MinMax
    from hgq.regularizers import MonoL1
    f0 = act_bits - 1 - int(i0)
    regularizer = MeanMonoL1 if heterogeneous_axis else MonoL1
    return QuantizerConfig("kif", "datalane", k0=1, i0=int(i0), f0=f0,
                           round_mode="RND_CONV", overflow_mode="SAT",
                           trainable=True, heterogeneous_axis=heterogeneous_axis,
                           ic=MinMax(-8, 12), fc=MinMax(-8, 16),
                           ir=regularizer(float(bw_l1)), fr=regularizer(float(bw_l1)))
...
def build_qat_model(cfg: dict, seed: int, enable_ebops: bool = True,
                    input_mu=None, input_sigma=None, with_pos_enc: bool = True):
    ...
    arch.pos_enc: "learned" adds the AddPositional table; "none" omits it (no position
    information at all). with_pos_enc=False drops the layer for the export graphs, which
    fold a learned table into input_proj's bias instead."""
    ...
    E = D // H
    if A["pos_enc"] not in ("learned", "none"):
        raise ValueError(f"unknown arch.pos_enc {A['pos_enc']!r}")
    ...
    if act_calib not in ("frozen", "trainable", "recalib", "free"):
        raise ValueError(f"{cfg['name']}: unsupported quant.act_calib={act_calib!r}")
    act_bw_l1 = float(cfg["quant"].get("act_bw_l1", 1e-8))
    beta0 = float(cfg["quant"].get("beta0", 0.0))
    taps: dict = {}
    ...
    def act_iq(i0=_PROV_I, axes=(-1,)):
        """Free width, fixed-width trainable scale, or a frozen activation grid."""
        if act_calib == "free":
            granularity = cfg["quant"].get("act_granularity", "tensor")
            if granularity not in ("tensor", "channel"):
                raise ValueError(f"Unsupported act_granularity: {granularity}")
            return _free_act(ab, i0, act_bw_l1,
    ...
    def dense_einsum(name, equation, out_shape, x, bias_axes=None):
        ...
        if fp32:
            ...
            kq, iq = _binary_kq(), act_iq(axes=(-2, -1) if len(x.shape) == 4 else (-1,))
            cls = BitQEinsumDense
        else:  # w8a8
    ...
    def stream_iq():
        return _dummy("datalane") if fp32 else act_iq(6, axes=(-2, -1))  # shared over particles and batch
    ...
    sm_out_bits = int(cfg["quant"].get("softmax_out_bits", max(ab, 10)))
    sm_out_i = int(cfg["quant"].get("softmax_out_i", 1))
    if sm_out_bits - sm_out_i < 1:
        raise ValueError("softmax output needs at least one fractional bit")
    ...
    with LayerConfigScope(enable_ebops=enable_ebops, beta0=beta0):
        ...
        for li in range(L):
            ...
            attn_iq = _dummy("datalane") if fp32 else \
                QuantizerConfig("kif", "datalane", k0=0, i0=sm_out_i, f0=sm_out_bits - sm_out_i,
                                round_mode="RND_CONV", overflow_mode="SAT",
                                trainable=False, heterogeneous_axis=())
...
def calibrate_activations(model, taps: dict, X: np.ndarray, act_bits: int,
    ...
    Policy = argmin_i MSE of the static grid (reference-model E4 finding: max+margin over-
