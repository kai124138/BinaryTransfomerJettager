...
The FINAL reference model graph (build.py) is EinsumDense-dominated: 51 QEinsumDense carry
6.4M ±1 weights, and hls4ml 1.3.0 makes EinsumDense **Latency-only**, so at Latency all
those weights inline as compiler constants → the 27M-IR monolith OOM'd on the synthesis host, and the
...
def emit_input_proj(x_in, cfg, binz, calib, pe, reg):
    ...
    Reinstates y=β̃·M[t,d]+bias_td[t,d]+PE[t,d]. The reference model's input_proj bias is
    ...
    the affine; a tiny (~3e-4) seed vs the reference model's dense-bias-TABLE quantization remains
    (the affine quantizes the additive directly, the reference model pre-divides by β̃ then rescales
...
def build_monolith_v2(cfg, binz, calib, pe, enable_ebops=True):
    """The full reference model model, hybrid emission. Returns (model, reg). Port with
...
def build_block_v2(cfg, binz, calib, li, enable_ebops=True):
    ...
    standalone block). Near-lossless (2^-16 grid) → the block C-sim vs the reference model stands."""
    import keras
    from hgq.config import LayerConfigScope
    from hgq.layers import QBatchNormalization
    A = cfg["arch"]
...
def latency_layer_names(L):
    ...
    softmax follows the reference model's all-Latency attention path)."""
    names = []
    for li in range(L):
        blk = f"bit_block_{li}"
