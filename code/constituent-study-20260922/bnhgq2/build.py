...
* Activation quant  = KIF(k0=1, i0, f0, RND, SAT, trainable=False) per site, i0
  from reference-model calibration, f0 = act_bits − 1 − i0.
...
def build_hgq2_model(cfg: dict, binz: dict, calib: dict, enable_ebops: bool = True):
    ...
    # passthrough: no layer, no params, no DSP -- the norm-ablation science variant). Guarded
    # on arch.norm so a config without norm=='none' emits a byte/hash-identical export graph.
    ...
    if norm_kind not in ("subln", "none"):
        raise ValueError(f"{cfg['name']}: unsupported arch.norm={norm_kind!r} "
                         "(export stack supports 'subln' or 'none')")
    # v5 : no LayerNorm exists to absorb any β,
    # so EVERY β is restored in-graph by its own affine right after its matmul —
    # Wq/Wk/Wv/fc1/head_fc1 gain affines (6L+3 total), downstream γs carry nothing.
    # Streams are then true-scale everywhere and the TRAINED act/attention grids
    # apply verbatim, clipping included (the pre-conference gate-1 root cause was re-derived
    # carry-site grids destroying trained saturation). Normed graphs: byte-identical.
    restore_beta = norm_kind == "none"
    ...
    with LayerConfigScope(enable_ebops=enable_ebops, beta0=0.0):
        ...
        for li in range(L):
            ...
            if restore_beta:
                # β_q/β_k/β_v back in-stream (shift-add constants) → the scores /
                # softmax / ctx grids can be the TRAINED QAT grids verbatim
                # (convert_binary.match_attention_to_qat portable copy, no scalers).
                q = affine(f"{blk}_attn_Wq", q)
                k = affine(f"{blk}_attn_Wk", k)
                v = affine(f"{blk}_attn_Wv", v)
            ...
            # normed: β_q·β_k folds into the softmax exp-LUT scale (score_fold).
            # norm-free v5: β_q/β_k are already in-stream via their affines →
            # 1/√d_head only, matching the QAT softmax input_scaler exactly.
            sscale = (1.0 / float(np.sqrt(E)) if restore_beta else
                      binz[f"{blk}_attn_Wq"]["beta"] * binz[f"{blk}_attn_Wk"]["beta"]
                      / float(np.sqrt(E)))
            ...
            fc1_bias = {} if restore_beta else dict(bias_axes="f", bq_conf=bq_wide())
            f1 = QEinsumDense("btd,df->btf", output_shape=(T, FFN),
                              iq_conf=act_q(i0(f"{blk}_ffn_fc1")),
                              kq_conf=kq(f"{blk}_ffn_fc1"),
                              name=f"{blk}_ffn_fc1", **fc1_bias)(ln)
            if restore_beta:
                # y = β_fc1·(±1·x) + b EXACTLY (bias in the affine's b term, not b/β
                # in the dense), then ReLU — commutes by positive homogeneity.
                f1 = affine(f"{blk}_ffn_fc1", f1)
        ...
        hf1_bias = {} if restore_beta else dict(bq_conf=bq_wide())
        h = QDense(D, use_bias=not restore_beta,
                   iq_conf=act_q(i0("head_fc1")), kq_conf=kq("head_fc1"),
                   name="head_fc1", **hf1_bias)(h)
        if restore_beta:
            h = affine("head_fc1", h)
