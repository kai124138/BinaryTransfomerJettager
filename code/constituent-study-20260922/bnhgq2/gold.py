...
Two activation-quantization modes:
  * 'dynamic' — exact QKeras training semantics (per-token absmax scale). Used to
    PROVE this reimplementation matches the trained model (vs roc-results/reference experiment npz).
...
def csd_snap(beta: float, ndig: int) -> float:
    """Best <=ndig-signed-digit approximation of beta. Digit window a0+1..a0-8
    (same 2^-16-exactness convention as csd2_snap). 3+ digits cost an extra
    compile-time add per constant and may tempt Vitis past the <=2-digit rule —
    measure DSPs on the synthesis host before shipping ."""
    import itertools
    a0 = int(np.floor(np.log2(beta)))
    positions = list(range(a0 + 1, a0 - 9, -1))
    best, err = 2.0 ** a0, abs(2.0 ** a0 - beta)
    for k in range(1, ndig + 1):
        for pos in itertools.combinations(positions, k):
            for signs in itertools.product((1.0, -1.0), repeat=k):
                v = sum(s * 2.0 ** p for s, p in zip(signs, pos))
                if v <= 0:
                    continue
                e = abs(v - beta)
                if e < err:
...
def csd3_snap(beta: float) -> float:
    return csd_snap(beta, 3)


def fx8_snap(beta: float) -> float:
    """beta rounded to an 8-fractional-bit constant (multiple of 2^-8): the
    cheapest encoding that cleared the norm-free gate-1 ladder (diag4 2026-08-04:
    n16 0.9975 / n8 0.9989 vs csd3 0.9965/0.9988, csd2 0.9912/0.9960)."""
