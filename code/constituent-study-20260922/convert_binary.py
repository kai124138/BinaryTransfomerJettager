...
def build_export(cfg, binz, calib, pe, beta_mode="csd2",
    ...
    if cfg.get('engram_study', {}).get('module') is not None:
        raise NotImplementedError('JetEngram has no HLS lowering yet; exporting just its backbone would change the model')
    if qat_model is not None and any(type(layer).__name__ == 'JetEngram' for layer in qat_model.layers):
        raise NotImplementedError('JetEngram checkpoint cannot be exported by the backbone-only converter')
