"""Prepare immutable N8/N64 caches from one existing training-data read."""
import argparse
import fcntl
import hashlib
import json
...
def main():
    ...
    parser.add_argument('--n-parts', default='8,64')
    ...
    for cfg in configs:
        ...
        by_n[n] = cfg
    assert set(requested) <= set(by_n) == {8, 64}
    ...
    for n in requested:
        ...
        with (cache / 'prepare.lock').open('a+b') as lock:
            ...
            if raw_x is None:
                ...
                print('[raw_load]', str(args.raw), 'N64 once', flush=True)
                raw_x, raw_y, nfiles = load_train_data(str(args.raw), 64, features=cfg['arch']['features'])
                assert raw_x.shape == (620000, 64, 3) and raw_y.shape == (620000, 5) and nfiles == 62
