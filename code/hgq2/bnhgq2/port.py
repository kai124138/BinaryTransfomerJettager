...
Kernels are assigned as the SIGN matrices (±1); each explicit site's β̃ lives in its
`_affine` layer (γ = CSD-2(β) at port time; csd3/fx8/exact re-assigned afterwards by
convert_binary._apply_beta_encoding). v5 norm-free : γ carries
ONLY its own site's β — every upstream β is restored at source by its own affine
(Wq/Wk/Wv/fc1/head_fc1 included), and fc1/head_fc1 denses are biasless with the exact
bias in the affine's b term.
"""
...
def port_weights(model, cfg: dict, binz: dict, pe: np.ndarray):
    ...
    for layer in model.layers:
        ...
        if nm.endswith("_affine"):
            ...
            src = nm[:-len("_affine")]
            e = binz[src]
            width = e["shape"][-1]  # output feature dim = last kernel axis (2D
            # QKeras-path kernels: shape[-1]==shape[1]; QAT einsum-native kernels
            # are 3D, e.g. Wo (H,E,D) -> D is the last axis, not shape[1]=E)
            # v5: γ carries ONLY this site's own β. Norm-free graphs restore every
            # upstream β at source (its own affine right after its matmul), so the
            # old downstream carry (γ_Wo·β_v etc.) is retired — it forced re-derived
            # carry-site input grids that destroyed the trained saturation (gate-1
            # 0.839, experimental protocol 2026-08-04). Normed graphs: identical to before
            # (their carry was 1 by construction).
            gamma = np.full(width, csd2_snap(e["beta"]), dtype=np.float32)
            beta = (np.zeros(width, np.float32)
                    if src == "input_proj" or e["bias"] is None
        ...
        if nm == "input_proj":
            ...
            # norm-free v5: biasless dense — the exact bias rides in the site's own
            # affine (γ·(±1·x) + b); normed: b/β in the dense (the next LN kills β).
            _assign(layer, q(nm), None if norm_free else b(nm))
        else:
            continue
        n_set += 1
    n_affines = (6 * A["n_layers"] + 3) if norm_free else (2 * A["n_layers"] + 2)
    expected = (3 + 6 * A["n_layers"]) + n_affines  # denses + affines
    if n_set != expected:
